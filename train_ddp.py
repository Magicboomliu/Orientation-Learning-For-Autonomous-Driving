...
from trainfiles.trainer_ddp import Trainer
...
def main(opt):
    ...
    trainer = Trainer(lr=opt.lr,backbone=int(opt.backbone),devices=opt.devices,
                      ...
                      test_size=opt.test_batch,pretrain=opt.pretrain,local_rank=opt.local_rank)
    ...
    for r in range(opt.startRound, train_round):
        ...
        for i in range(start_epoch, end_epoch):
            ...
            avg_loss, avg_acc,iterations = trainer.train_one_epoch(i, r,iterations,summary_writer,opt.local_rank)
            val_acc = trainer.validate(summary_writer,i,local_rank=opt.local_rank)
